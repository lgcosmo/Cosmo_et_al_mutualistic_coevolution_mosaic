...
function moore_neighborhood(;n_patches, n, periodic=true)
...
function occupancy(;z::Array{Float64})
...
function sp_extinctions2(;O::Array{Float64})
  ...
  @inbounds for i in 1:size(O,2)
        ...
        if any(O_i.==0.0)
            ext[i]+=1.0
            time_ext[i]=findfirst(x-> x==0.0, O_i)
        end
    end
...
function T_matrix(;A::Array{Float64}, QS::Array{Float64})
...
function create_dirs(;prop_hot, nsim)
    for i in 1:length(prop_hot)
        for j in 1:nsim
            mkpath(datadir("sims", "prophot_$(prop_hot[i])", "sim_$(j)", "spatial_networks"))
        end
    end
end
